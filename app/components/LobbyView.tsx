...
export default function LobbyView({
  ...
  return (
    <div className="w-full flex flex-col justify-start space-y-4 sm:space-y-5 flex-1 max-w-sm mx-auto pb-4 pt-2 relative">
      ...
      {/* 2x2 카테고리 타일 그리드 유지 (쫀득한 애니메이션 포함) */}
      ...
      {/* ★ 세부 주제 선택 바텀 시트 모달 (디자인 정밀 수정본) */}
      {activeModalCat && (
        // ★ 수정: paddingBottom을 calc(env(safe-area-inset-bottom) + 60px)으로 설정하여 내비게이션 바 위로 '붕 띄우도록' 함 (디자인 개선)
        <div className="fixed inset-0 z-50 flex flex-col justify-end bg-black/60 backdrop-blur-sm transition-opacity duration-300" style={{ paddingBottom: 'calc(env(safe-area-inset-bottom) + 60px)' }}>
          <div 
            // ★ 수정: pb-28을 pb-6으로 줄여 상단 콘텐츠가 아래로 내려오게 함 (쏠림 문제 해결) + max-h 유지로 내부 스크롤 활성화
            className="bg-[#111] w-full max-w-lg mx-auto rounded-t-3xl border-t border-x border-white/10 p-6 pb-6 flex flex-col max-h-[85vh] animate-[slide-up_0.3s_ease-out_forwards]"
