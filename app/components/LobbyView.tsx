...
export default function LobbyView({
  ...
  isDropdownOpen, setIsDropdownOpen, isConnecting, isSingleMode, handleMatchStart, setStep
}: any) {
  
  const currentOptions = LOBBY_CATEGORIES.find(c => c.id === selectedCategory)?.options || [];

  return (
    // ★ pb-24를 주어서 맨 아래 버튼이 하단바에 먹히지 않게 스크롤 여백 확보
    <div className="w-full flex flex-col justify-center space-y-6 flex-1 min-h-[500px] pb-24">
      
      {/* 1. 메인 타이틀 영역 (오리지널 복구) */}
      <div className="text-center space-y-1 mb-1 shrink-0 mt-4">
      ...
      <div className="bg-white/[0.03] backdrop-blur-xl p-5 rounded-3xl border border-white/5 shadow-2xl space-y-4 shrink-0">
        ...
        {/* 광장 버튼 추가 (오리지널 디자인 톤에 맞춤) */}
        <button 
          onClick={() => setStep('lounge')} 
          className="w-full bg-[#0f172a] hover:bg-[#1e3a8a] text-blue-200 font-extrabold tracking-widest py-3.5 rounded-xl border border-blue-500/30 transition-all shadow-lg flex justify-center items-center gap-2 text-[14px] mt-2"
        >
