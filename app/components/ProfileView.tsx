...
export default function ProfileView({ userId, tier, personaTitle, socketRef }: any) {
  ...
  const [friendsList, setFriendsList] = useState<{ userId: string, nickname: string }[]>([]);

  // 서버에서 프로필 정보(닉네임, 친구) 수신
  useEffect(() => {
  ...
  return (
    <div className="w-full flex flex-col h-full bg-[#080808]/90 backdrop-blur-2xl border border-white/5 rounded-[2rem] p-6 shadow-2xl relative mb-6">
      ...
      {/* 1. 상단: 내 프로필 정보 및 닉네임 수정 */}
      <div className="flex flex-col items-center justify-center space-y-4 pb-6 border-b border-white/10 shrink-0 mt-4">
        <div className="w-20 h-20 rounded-full bg-gradient-to-tr from-emerald-500/20 to-blue-500/20 border border-white/20 flex items-center justify-center text-3xl shadow-[0_0_30px_rgba(16,185,129,0.15)]">
          🎭
        </div>
          ...
          {isEditing ? (
            <div className="flex items-center justify-center gap-2 mt-2">
              <input 
                ...
                maxLength={10}
                className="bg-black border border-emerald-500/50 text-white text-center rounded-lg px-3 py-1.5 w-32 outline-none focus:border-emerald-400 text-sm"
                autoFocus
              />
            ...
            <div className="flex items-center justify-center gap-2 mt-2">
              ...
              <button onClick={handleEditClick} className="text-white/40 hover:text-white/80 text-sm">✏️</button>
            </div>
          )}
      ...
      <div className="flex-1 flex flex-col mt-6 overflow-hidden">
        <div className="flex justify-between items-center mb-4 shrink-0 px-2">
          <div className="flex items-center gap-2">
            ...
            <span className="text-white/80 text-xs font-bold tracking-widest uppercase">인사이트 인맥</span>
          </div>
          <span className="text-[10px] bg-white/10 text-white/60 px-2 py-0.5 rounded-full">{friendsList.length}명</span>
        </div>
          ...
          {friendsList.length === 0 ? (
            <div className="h-full flex flex-col items-center justify-center text-center opacity-50">
              ...
              <p className="text-[11px] text-white/70 leading-relaxed">
                아직 등록된 인맥이 없습니다.<br/>대화가 잘 통했던 파트너를 추가해 보세요.
              </p>
            </div>
          ) : (
              ...
              <div key={idx} className="bg-black/40 border border-white/5 rounded-xl p-4 flex items-center justify-between hover:bg-white/5 transition-colors cursor-pointer group">
                <div className="flex items-center gap-3">
                  ...
                  <div className="flex flex-col text-left">
                    <span className="text-sm font-bold text-white/90 group-hover:text-emerald-300 transition-colors">{friend.nickname}</span>
                    <span className="text-[9px] text-white/30 font-mono truncate w-32">{friend.userId}</span>
                  </div>
                </div>
                <button className="text-[10px] border border-white/10 bg-white/5 text-white/60 px-3 py-1.5 rounded-full hover:bg-white/20 hover:text-white transition-all">
                  쪽지
                </button>
              </div>
            ))
